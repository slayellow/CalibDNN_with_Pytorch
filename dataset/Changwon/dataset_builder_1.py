...
for path, dirs, files in os.walk(dataset_path):
    for foldername in dirs:
        if not os.path.exists(dataset_path + foldername + "/depth_maps"):
            os.makedirs(dataset_path + foldername + "/depth_maps")
        if not os.path.exists(dataset_path + foldername + "/depth_maps_transformed"):
            os.makedirs(dataset_path + foldername + "/depth_maps_transformed")
        if not os.path.exists(dataset_path + foldername + "/target_images"):
            os.makedirs(dataset_path + foldername + "/target_images")
        ...
        for img_name, cloud_name in zip(imgs_files, point_files):
            ...
            smc.imsave(dataset_path + foldername + "/depth_maps" + "/" + imagefile, pooled_img)
            smc.imsave(dataset_path + foldername + "/target_images" + "/" + imagefile, reconstructed_img)

        np.savetxt(dataset_path + foldername + "/angle_list.txt", angle_list[1:], fmt = "%.4f")