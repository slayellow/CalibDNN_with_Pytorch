...
# dataset test code
trainingset = CalibDNNDataset_Changwon(cf.paths['dataset_changwon_path'], training=True)
...
model = CalibDNN18(18, pretrained = os.path.join(pretrained_path, "CalibDNN_18" + '.pth')).to(devices)
print("------------ Model Summary ----------------")
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[35, 45, 55], gamma=0.5)


if os.path.isfile(os.path.join(pretrained_path, "CalibDNN_18" + '.pth')):
    print("Pretrained Model Open : ", model.get_name() + ".pth")
    checkpoint = load_weight_file(os.path.join(pretrained_path, "CalibDNN_18" + '.pth'))
...
for epoch in range(start_epoch, cf.network_info['epochs']):
    ...
    save_checkpoint({
        ...
        'optimizer': optimizer.state_dict()}, True, os.path.join(pretrained_path, 'CalibDNN_18_ALL'))
